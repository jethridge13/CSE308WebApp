    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            */
            ...
            String password = request.getQueryString().substring(request.getQueryString().lastIndexOf("=") + 1);
            id = request.getQueryString().substring(request.getQueryString().indexOf("=") + 1, 
                    request.getQueryString().indexOf("&"));
            try {
                User user = User.getUser(id, password);
                if(user.isStudent()){
                    out.write("Student");
